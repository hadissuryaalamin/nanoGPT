...
def run_external_eval_and_get_ppl():
    """
    Run eval.py using the latest checkpoint in out_dir and parse PPL from stdout.
    Expected stdout contains one of:
      - 'ppl 24.73'
      - 'ppl: 24.73'
      - 'perplexity 24.73'
      - 'perplexity: 24.73'
    ...
    # optional: robust if someday eval.py prints PPL_RESULT=...
    match = re.search(r"PPL_RESULT=([0-9]+(?:\.[0-9]+)?)", stdout)
    if match:
        ...
        if ppl is not None:
            ...
            if ppl < best_eval_ppl:
                ...
                if os.path.exists(src):
                    ...
                    print(f"New best ppl {best_eval_ppl:.4f}, saved best checkpoint to {dst}")
