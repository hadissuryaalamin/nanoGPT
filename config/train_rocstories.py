...
n_layer = 16         # larger for better reasoning(complexity understanding)
n_head = 16         # larger for more various attention patterns(better prespectives) 
n_embd = 256        # larger for more representation
...
learning_rate = 5e-3                # larger for faster training, smaller for better accuracy
max_iters = 3000                    # larger for longer training(might increase accuracy), smaller for faster runs
lr_decay_iters = max_iters - 500    # Starting step for learning rate decay
min_lr = 5e-4                       # learning rate after decay
warmup_iters = 500                  # larger for more stable training - might spike at the start 
weight_decay = 0.15                 # larger for more regularization(less overfitting), smaller for less
...
grad_clip = 0.5                     # larger for more stable training(gradient might be exploding), smaller for faster convergence
